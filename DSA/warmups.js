...
//PROBLEM 
//https://the-winter.github.io/codingjs/exercise.html?name=sameStarChar&title=String-2
...
// iterate from 1 to str. length - 1. 
// if we have star, check to see if chacter before and after are the same. If not the same return false. 
// if we finish iteration, return true.
function sameStarChar(str) {
  for(let i = 1; i < str.length - 1; i++) {
    let char = str[i];
    if(char === "*" && (str[i - 1] !== str[i + 1])) return false; 
  }
  return true; 
}